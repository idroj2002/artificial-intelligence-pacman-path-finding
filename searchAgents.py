...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        _, visited_old = state
        successors = []
        for action, next_pos in self.getActions(state):
            visited = tuple(
                cv or cp == next_pos
                for cp, cv in zip(
                    self.corners,
    ...
    def getActions(self, state) -> Sequence[Tuple[str, Tuple[int, int]]]:
        possible_directions = [
            Directions.NORTH,
            Directions.SOUTH,
            Directions.EAST,
        ...
        for action in possible_directions:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                valid_actions_from_state.append((action, (nextx, nexty)))
